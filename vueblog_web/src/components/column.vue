...
<script>
  export default {
    ...
    methods:{
      //获取栏目所有信息
     async getcolumn (){
        const {data:res}= await this.$http.get("getcolumn")
        // console.log(res)
      ...
      //添加栏目
      async addcolumn(){
        const {data:res}=await this.$http.get("addcolumn",{ params:{ column:this.inputcolumn } })
    ...
    async  handleDeletetest(cid){
       console.log(cid)
        this.cid=cid
    const {data:res} =await this.$http.get("deletecolumn",{ params:{cid:this.cid}})
      if(res==="删除成功"){
